...
class TraderToTraderController extends Controller
{
    public function __invoke(Request $request)
    {
        try {
            ...
            // check receiver trader found or not
            if (!$receiver) {
                return Response::json([
                    'status' => false,
                    'otp' => false,
            ...
            // everything is good now create transaction
            else {
                ...
                if ($created) {
                    ...
                    $last_transaction = ExternalFundTransfers::where('sender_id', $trader_user->id)->where('type', 'trader_to_trader')->latest()->first();
    ...
    public function otp_check(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                try {
                    $user_otp = OtpCode::where('user_id', $trader_user->id)
                        ->where('type', 'trader_to_trader')->first();
            ...
            if ($code->created_at->addMinutes(20) < Carbon::now()) {
                $code->delete();
                return Response::json(
                    [
                        'status' => false,
