...
class IbClientDepositReport extends Controller
{
    public function __construct()
    {
        $this->middleware(AllFunctionService::access('deposit_reports', 'ib'));
    ...
    public function depositReports(Request $request)
    {
        ...
        $methods = Deposit::select('transaction_type')->distinct()->get();
        // return $methods;
        return view('ibs.affiliate.client_deposit_report',compact('methods'));
    }
    // deposit reqport datatable
    public function depositReportDT(Request $request)
    {
        try {
            ...
            $result = Deposit::select('deposits.*', 'users.name', 'users.email')
                ->join('users', 'deposits.user_id', '=', 'users.id')
                ->whereIn('deposits.user_id', $traders);
                ...
                if ($dts->search) {
                    foreach ($columns as $col) {
                        if ($col['data'] != 'id' && $col['data'] != 'ib' && !empty($col['data'])) {
                            $tf = $col['data'] == 'created_at' ? 'deposits.created_at' : $col['data'];
                            $st = $dts->search;
            ...
            // filter by status
            if ($request->status != "") {
                $result = $result->where('deposits.approved_status', $request->status);
            }
            ...
            if ($request->ib_info != "") {
                ...
                $result = $result->whereIn('deposits.user_id', $filter_client);
            }
            ...
            if ($request->deposit_method != "") {
                $result = $result->where('transaction_type', $request->deposit_method);
            }
            ...
            // filter by date from
            if ($request->from != "") {
                $result = $result->whereDate('deposits.created_at', '>=', Carbon::parse($request->from)->format('Y-m-d'));
            }
            // filter by date to
            if ($request->to != "") {
                $result = $result->whereDate('deposits.created_at', '<=', Carbon::parse($request->to)->format('Y-m-d'));
            }
            ...
            foreach ($result as $row) {
                ...
                $data[$i]["amount"] = round($row->amount, 2);
                $i++;
            }
