...
class BalanceTransferToTraderReportController extends Controller
{
    ...
    public function balanceTransferReports(Request $request)
    {
        ...
        return view('ibs.reports.balance_transfer_ib_to_trader_report');
    }
        ...
        try {
            ...
            $result = ExternalFundTransfers::select(
                ...
                ->where(function ($query) {
                    $query->where('sender_id', auth()->user()->id)
                        ->where('external_fund_transfers.type', 'ib_to_trader');
                });
            ...
            foreach ($result as $value) {
                ...
                if ($value->sender_id == auth()->user()->id) {
                    $transaction_type = '<span class="badge badge-warning badge-sm">Send</span>';
                } else {
                    $transaction_type = '<span class="badge badge-success badge-sm">Receive</span>';
                }
                $data[$i]["name"] = ucwords($value->name);
                $data[$i]["email"] = $value->email;
                $data[$i]['type'] = $transaction_type;
