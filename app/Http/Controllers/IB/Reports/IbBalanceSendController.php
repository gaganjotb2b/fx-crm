...
class IbBalanceSendController extends Controller
{
    ...
    {
        $this->middleware(AllFunctionService::access('ib_balance_send', 'ib'));
        $this->middleware(AllFunctionService::access('reports', 'ib'));
        $this->middleware('is_ib');
    }
    public function BalanceSendingReports(Request $request)
    {
        if ($request->action === 'table') {
            return $this->balanceTransferReportDT($request);
        }
        return view('ibs.reports.balance_send_report');
    }
        ...
        try {
            ...
            $result = ExternalFundTransfers::select(
                ...
                ->where(function ($query) {
                    $query->where('external_fund_transfers.type', 'ib_to_trader')
            ...
            foreach ($result as $value) {
                ...
                if ($value->receiver_wallet_type === 'ib') {
                    $reciver_type = '<span class="badge badge-warning badge-sm">IB</span>';
                } else {
                    $reciver_type = '<span class="badge badge-success badge-sm">Trader</span>';
                }
                $data[$i]["name"] = ucwords($value->name);
                $data[$i]["email"] = $value->email;
                $data[$i]["status"] = $status;
                $data[$i]["reciver_type"] = $reciver_type;
