...
class MyInvestmentController extends Controller
{
    ...
    {
        try {
            return view('traders.pamm.non-copy-pamm.reports.my-investment');
        } catch (\Throwable $th) {
            //throw $th;
        }
    }
    public function investment_report(Request $request)
    {
        try {
            ...
            $result = InternalTransfer::with(['account', 'user', 'account.pammUser'])
            ...
            foreach ($result as $value) {
                ...
                $data[] = [
                    ...
                    'pamm_email' => $value->account?->pammUser?->email ?? '---',
