...
class adminRegistratoinMail extends Mailable
{
    ...
    public function build()
    {
        ...
        $template = EmailTemplate::select('name')->where('use_for', 'admin-registration')->first();
        if ($template && $template->name != null) {
            return $this->view('email.' . $template->name)
                ->with($data);
        } else {
            return $this->view('email.mail-admin-regitration')->with($data);
        }
    }
}
