$(function () {
  ...
  var form = $('.validate-form'),
    ...
    darkLogoUploadImg = $('#dark-logo-img'),
    darkLogoUploadBtn = $('#dark-logo-upload'),
    darkLogoUserImage = $('.upload-dark-logo'),
    darkLogoResetBtn = $('#dark-logo-reset'),
  ...
  if (darkLogoUserImage) {
    var resetImage = darkLogoUserImage.attr('src');
    darkLogoUploadBtn.on('change', function (e) {
      var reader = new FileReader(),
        files = e.target.files;
      reader.onload = function () {
        if (darkLogoUploadImg) {
          darkLogoUploadImg.attr('src', reader.result);
        }
      };
      reader.readAsDataURL(files[0]);
    });

    darkLogoResetBtn.on('click', function () {
      darkLogoUserImage.attr('src', resetImage);
    });
  }
