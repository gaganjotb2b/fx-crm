...
$(function () {
    ...
    // Check if we're on the lead admin page
    var isLeadAdminPage = window.location.pathname.includes('/admin/client-management/lead-admin');
    
    // Alternative detection methods
    var isLeadAdminPageAlt1 = window.location.pathname === '/admin/client-management/lead-admin';
    var isLeadAdminPageAlt2 = window.location.href.includes('/admin/client-management/lead-admin');
    var isLeadAdminPageAlt3 = document.title.includes('Lead admin report');
    
    // Debug logging
    console.log('Lead Admin Debug:', {
        'current_path': window.location.pathname,
        'is_lead_admin_page': isLeadAdminPage,
        'includes_check': window.location.pathname.includes('/admin/client-management/lead-admin'),
        ...
        'alternative_checks': {
            'exact_path_match': isLeadAdminPageAlt1,
            'href_includes': isLeadAdminPageAlt2,
            'title_includes': isLeadAdminPageAlt3
        }
    });
    
    // Use the most reliable detection method
    var finalIsLeadAdminPage = isLeadAdminPage || isLeadAdminPageAlt1 || isLeadAdminPageAlt2 || isLeadAdminPageAlt3;
    console.log('Final lead admin detection:', finalIsLeadAdminPage);
    ...
    var datatable = dt_ajax_table.fetch_data({
        ...
        icon_feather: true,
        customorder: 3,
        // Add is_lead_admin parameter for lead admin page
        data: function(d) {
            if (finalIsLeadAdminPage) {
                d.is_lead_admin = true;
                console.log('Setting is_lead_admin = true for AJAX request');
            } else {
                console.log('Not on lead admin page, not setting is_lead_admin');
            }
            console.log('AJAX data object:', d);
        },
        columns: [
            { "data": "select" },
            { "data": "name" },
