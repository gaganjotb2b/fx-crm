var datatable = $('.datatables-ajax').fetch_data({
    url: '/admin/settings/ib-setting/create-all-dt',
    columns: [
        { data: 'action' },
        { data: 'title' },
    ],
...
$(document).on('click', '#create-all-permission', function () {
    ...
    Swal.fire({
        title: 'Create IB permission',
        text: 'Are You Confirm to reset IB permission ?',
        inputAttributes: {
            autocapitalize: 'off'
        },
        ...
        preConfirm: (login) => {
            ...
            return fetch('/admin/settings/ib-setting/create-all')
                .then(response => {
                    if (!response.ok) {
        ...
        if (result.isConfirmed) {
            if (result.value.status == false) {
                notify('error', result.value.message, 'IB Permission');
            } else {
                datatable.draw()
                notify('success', result.value.message, 'IB permission');
            }
        }
    })
})
...
$(document).on('change', '.switch-trader-settings', function () {
    ...
    Swal.fire({
        ...
        preConfirm: (login) => {
            $(".swal2-html-container").text("Please wait while we create permission...")
            return fetch('/admin/settings/ib-setting/create-all?op=update&id=' + id)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(response.statusText)
                    }
