...
@section('title', 'Lead admin report')
@section('vendor-css')
...
<style>
    ...
    .card-statistics h4 {
        ...
        margin: 0;
        line-height: 1.2;
        background: linear-gradient(45deg, #7367f0, #9c8cfc);
    ...
    .bg-light-primary .avatar-icon {
        color: #7367f0 !important;
    }
    
    .bg-light-success .avatar-icon {
        color: #28c76f !important;
    }
    
    .bg-light-info .avatar-icon {
        color: #00cfe8 !important;
    }
    
    .bg-light-warning .avatar-icon {
    ...
    .dark-layout .card-statistics:hover {
        box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.32);
    }
    
    .dark-layout .card-statistics .card-text {
        color: #b4b7bd;
    }
    ...
    /* Assigned/Unassigned Leads Cards Styling */
    .card-assigned-leads, .card-unassigned-leads {
        transition: all 0.3s ease;
        border: none;
    ...
    .card-unassigned-leads {
        background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);
        box-shadow: 0 8px 32px rgba(245, 158, 11, 0.3);
    }
    
    .card-assigned-leads:hover, .card-unassigned-leads:hover {
        transform: translateY(-3px);
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.15);
    }
    
    .assigned-icon-wrapper, .unassigned-icon-wrapper {
        flex-shrink: 0;
    }
    
    .assigned-icon, .unassigned-icon {
        width: 60px;
        height: 60px;
        border-radius: 50%;
        display: flex;
    ...
    .assigned-icon-inner, .unassigned-icon-inner {
        color: white;
        font-size: 1.5rem;
        font-weight: bold;
    }
    
    .assigned-content, .unassigned-content {
        flex: 1;
    }
    ...
    .assigned-progress, .unassigned-progress {
        width: 100%;
        height: 4px;
        background: rgba(255, 255, 255, 0.2);
        border-radius: 2px;
    ...
    /* Dark theme support for Assigned/Unassigned cards */
    .dark-layout .card-assigned-leads {
        background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%);
    }
    
    .dark-layout .card-unassigned-leads {
        background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);
    }
    
    /* Mobile responsiveness for Assigned/Unassigned cards */
    @media (max-width: 768px) {
        .assigned-icon, .unassigned-icon {
            width: 50px;
            height: 50px;
    ...
    .card-kyc-verified, .card-kyc-unverified {
        transition: all 0.3s ease;
        border: none;
        border-radius: 0.75rem;
        overflow: hidden;
    ...
    .card-kyc-verified:hover, .card-kyc-unverified:hover {
        transform: translateY(-3px);
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.15);
    }
    ...
    .progress-bar-kyc-verified, .progress-bar-kyc-unverified {
        height: 100%;
        background: rgba(255, 255, 255, 0.8);
        border-radius: 2px;
        transition: width 0.8s ease;
    ...
    /* Dark theme support for KYC cards */
    .dark-layout .card-kyc-verified {
        background: linear-gradient(135deg, #10b981 0%, #059669 100%);
    }
    
    .dark-layout .card-kyc-unverified {
        background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
    }
    
    /* Mobile responsiveness for KYC cards */
    @media (max-width: 768px) {
        .kyc-icon {
            width: 50px;
            height: 50px;
    ...
    /* Country Graph Card Styling - Classic Design */
    .card-country-graph {
    ...
    .card-country-graph:hover {
        transform: translateY(-2px);
        box-shadow: 0 8px 25px rgba(44, 62, 80, 0.3);
    }
    
    .card-country-graph .card-title {
        color: #ffffff;
        font-weight: 600;
        font-size: 1.1rem;
        margin: 0;
    ...
    .card-country-graph .avatar-content {
        display: flex;
        align-items: center;
        justify-content: center;
        width: 100%;
    ...
    .card-country-graph .avatar-icon {
        color: #fff;
        font-size: 1.2rem;
    }
    ...
    .card-trend-compact:hover {
        transform: translateY(-3px);
        box-shadow: 0 10px 25px rgba(17, 153, 142, 0.35);
    }
    
    .card-trend-compact .card-title {
        color: #ffffff;
        font-weight: 600;
        font-size: 1.1rem;
        margin: 0;
    ...
    .card-trend-compact .avatar-content {
        display: flex;
        align-items: center;
        justify-content: center;
        width: 100%;
    ...
    .card-trend-compact .avatar-icon {
        color: #fff;
        font-size: 1.2rem;
    }
    ...
    /* Leads Graph Card Styling - New Design */
    .card-leads-graph {
    ...
    .card-leads-graph:hover::before {
        transform: translateX(100%);
    }
    
    .card-leads-graph:hover {
        transform: translateY(-8px) scale(1.02);
        box-shadow: 0 15px 40px rgba(0, 184, 148, 0.4);
    }
    
    .card-leads-graph .card-title {
        color: #ffffff;
        font-weight: 800;
        font-size: 1.5rem;
    ...
    .card-leads-graph .card-body {
        flex: 1;
        display: flex;
        flex-direction: column;
    }
    
    .card-leads-graph .graph-container {
        position: relative;
        z-index: 3;
        height: 280px;
    ...
    .card-leads-graph .graph-content {
        height: 100%;
        display: flex;
        flex-direction: column;
        justify-content: space-between;
    ...
    .card-leads-graph .graph-bar:hover {
        transform: scaleY(1.1);
        box-shadow: 0 4px 12px rgba(253, 203, 110, 0.5);
    }
    
    .card-leads-graph .bar-label {
        position: absolute;
        bottom: -25px;
        left: 50%;
    ...
    .card-leads-graph .graph-labels {
        display: flex;
        justify-content: space-around;
        margin-top: 10px;
    }
    
    .card-leads-graph .trend-indicator {
        display: flex;
        align-items: center;
        justify-content: center;
        margin-top: 15px;
    ...
    /* Mobile responsiveness for Country cards */
    @media (max-width: 768px) {
        .country-icon {
            width: 50px;
            height: 50px;
        ...
        .card-country-details .card-title {
            font-size: 1rem;
        }
...
<div class="app-content content page-trader-admin">
        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Lead Admin
                        </h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Lead Admin</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <section id="ajax-datatable">
                    <div class="row">
                        <div class="col-12">
                                    ...
                                    <div class="card card-statistics">
                                        <div class="card-body">
                                            <div class="d-flex flex-row">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['total_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My Assigned Leads
                                                        @else
                                                            Total Leads
                                                        @endif
                                    ...
                                    <div class="card card-statistics">
                                        <div class="card-body">
                                            <div class="d-flex flex-row">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['this_week_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My This Week Leads
                                                        @else
                                    ...
                                    <div class="card card-statistics">
                                        <div class="card-body">
                                            <div class="d-flex flex-row">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['this_month_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My This Month Leads
                                                        @else
                                    ...
                                    <div class="card card-statistics">
                                        <div class="card-body">
                                            <div class="d-flex flex-row">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['six_months_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My 6 Months Leads
                                                        @else
                            ...
                            <!-- Assigned/Unassigned/KYC Status Cards - Single Row -->
                            <div class="row mb-2">
                                <div class="col-xl-3 col-lg-6 col-md-6 col-12 mb-2">
                                    <div class="card card-statistics bg-light-primary">
                                        <div class="card-body">
                                            <div class="d-flex flex-row align-items-center">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['assigned_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My Assigned Leads
                                                        @else
                                ...
                                <div class="col-xl-3 col-lg-6 col-md-6 col-12 mb-2">
                                    <div class="card card-statistics bg-light-warning">
                                        <div class="card-body">
                                            <div class="d-flex flex-row align-items-center">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['unassigned_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My Unassigned Leads
                                                        @else
                                ...
                                <div class="col-xl-3 col-lg-6 col-md-6 col-12 mb-2">
                                    <div class="card card-statistics bg-light-success">
                                        <div class="card-body">
                                            <div class="d-flex flex-row align-items-center">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['kyc_verified_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My KYC Verified Leads
                                                        @else
                                                            KYC Verified Leads
                                                        @endif
                                                    </p>
                                ...
                                <div class="col-xl-3 col-lg-6 col-md-6 col-12 mb-2">
                                    <div class="card card-statistics bg-light-info">
                                        <div class="card-body">
                                            <div class="d-flex flex-row align-items-center">
                                                ...
                                                <div class="my-auto">
                                                    <h4 class="fw-bolder mb-0">{{ number_format($leadStats['kyc_unverified_leads'] ?? 0) }}</h4>
                                                    <p class="card-text font-small-3 mb-0">
                                                        @if(isset($leadStats['is_manager_view']) && $leadStats['is_manager_view'])
                                                            My KYC Unverified Leads
                                                        @else
                                                            KYC Unverified Leads
                                                        @endif
                                                    </p>
                                                </div>
                                            </div>
                            ...
                            <!-- Country Details Card - Horizontal Bar Graph -->
                            <div class="row mb-2">
                                <div class="col-xl-12 col-lg-12 col-md-12 col-12 mb-2">
                                    <div class="card card-country-graph">
                                        <div class="card-body">
                                            <div class="d-flex align-items-center mb-4">
                                                ...
                                                <h5 class="card-title mb-0">Country Wise Leads</h5>
                                            </div>
                                            <div class="country-graph-container">
                                                @if($leadStats['country_wise_leads'] && $leadStats['country_wise_leads']->count() > 0)
                                                    @php
                                                        $maxLeads = $leadStats['country_wise_leads']->max('lead_count');
                                                    @endphp
                                                    @foreach($leadStats['country_wise_leads']->take(8) as $country)
                                                        <div class="country-bar-item">
                                                            <div class="country-info">
                                                                <span class="country-name">{{ $country->country_name }}</span>
                                                                <span class="country-count">{{ $country->lead_count }}</span>
                                                            </div>
                                                            <div class="bar-container">
                                                                <div class="bar-fill" style="width: {{ $maxLeads > 0 ? ($country->lead_count / $maxLeads) * 100 : 0 }}%"></div>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                @else
                                                    <p class="text-muted small mb-0">No country data available</p>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: 'You want to convert this trader to lead.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Conver to lead',
    }).then((result) => {
        if (result.isConfirmed) {
            // Make the AJAX call to delete the trader
            $.ajax({
                url: `/admin/client-management/conver-to-lead-admin/${traderId}`, // The route to your delete function
...
$(document).ready(function () {
    ...
    // Console Log: Display traders data in browser console
    function logTradersData() {
        console.log('=== TRADERS LIST CONSOLE LOG ===');
        console.log('Page URL:', window.location.href);
        console.log('Current Time:', new Date().toLocaleString());
        
        // Get the DataTable instance
        var table = $('#root-table').DataTable();
        if (table) {
            var data = table.data().toArray();
            console.log('Total Traders in Current Page:', data.length);
            console.log('Traders Data:', data);
            
            // Log each trader individually for better readability
            data.forEach(function(trader, index) {
                console.log(`Trader ${index + 1}:`, {
                    'Name': trader.name || 'N/A',
                    'Email': trader.email || 'N/A',
                    'Phone': trader.phone || 'N/A',
                    'Status': trader.status || 'N/A',
                    'Account Manager': trader.account_manager || 'N/A',
                    'Category': trader.category || 'N/A',
                    'IP Address': trader.ip || 'N/A',
                    'Joined': trader.joined || 'N/A'
                });
            });
            
            // Special IP Address Debug
            console.log('=== IP ADDRESS DEBUG ===');
            data.forEach(function(trader, index) {
                console.log(`Trader ${index + 1} IP:`, {
                    'Name': trader.name || 'N/A',
                    'IP_Value': trader.ip,
                    'IP_Type': typeof trader.ip,
                    'IP_Length': trader.ip ? trader.ip.length : 0,
                    'Has_IP': !!trader.ip && trader.ip !== 'N/A',
                    'Raw_IP_Data': trader.ip,
                    'IP_Comparison': {
                        'Is_Empty_String': trader.ip === '',
                        'Is_Null': trader.ip === null,
                        'Is_Undefined': trader.ip === undefined,
                        'Is_N_A': trader.ip === 'N/A',
                        'Is_127_0_0_1': trader.ip === '127.0.0.1'
                    }
                });
            });
            
            console.log('=== END TRADERS LIST ===');
        } else {
            console.log('DataTable not initialized yet');
        }
    }
    
    // Log traders data when page loads
    $(document).ready(function() {
        setTimeout(function() {
            logTradersData();
        }, 2000); // Wait for DataTable to load
    });
    
    // Log traders data when DataTable is redrawn (pagination, search, etc.)
    $('#root-table').on('draw.dt', function () {
        setTimeout(function() {
            logTradersData();
    ...
    // Add a button to manually log traders data
    $('<button id="log-traders-btn" class="btn btn-info btn-sm ms-2">Log Traders to Console</button>').insertAfter('#root-table_filter label');
    
    $(document).on('click', '#log-traders-btn', function() {
        logTradersData();
    });
    
    // Enhanced logging function with table format
    function logTradersTable() {
        var table = $('#root-table').DataTable();
        if (table) {
            var data = table.data().toArray();
            
            console.log('%c=== TRADERS LIST TABLE FORMAT ===', 'background: #4CAF50; color: white; font-size: 16px; padding: 5px;');
            console.table(data.map(function(trader, index) {
                return {
                    'Sr No': index + 1,
                    'Name': trader.name || 'N/A',
                    'Email': trader.email || 'N/A',
            ...
            // Summary statistics
            console.log('%c=== SUMMARY STATISTICS ===', 'background: #2196F3; color: white; font-size: 14px; padding: 5px;');
            console.log('Total Traders:', data.length);
            console.log('Active Traders:', data.filter(t => t.status && t.status.includes('Active')).length);
            console.log('Inactive Traders:', data.filter(t => t.status && t.status.includes('Inactive')).length);
            console.log('Blocked Traders:', data.filter(t => t.status && t.status.includes('Block')).length);
        }
    }
    
    // Add button for table format logging
    $('<button id="log-traders-table-btn" class="btn btn-success btn-sm ms-2">Log Traders Table</button>').insertAfter('#log-traders-btn');
    
    $(document).on('click', '#log-traders-table-btn', function() {
        logTradersTable();
    });
    
    // Quick count display function
    function showTradersCount() {
        var table = $('#root-table').DataTable();
        if (table) {
            var data = table.data().toArray();
            var totalCount = table.page.info().recordsTotal;
            var currentPageCount = data.length;
            
            Swal.fire({
                title: 'Traders Count',
                html: `
                    <div class="text-center">
                        <h4>Total Traders: ${totalCount}</h4>
                        <p>Current Page: ${currentPageCount} traders</p>
                        <p>Page: ${table.page.info().page + 1} of ${table.page.info().pages}</p>
                    </div>
                `,
                icon: 'info',
                confirmButtonText: 'OK'
    ...
    // Add button for quick count display
    $('<button id="show-count-btn" class="btn btn-warning btn-sm ms-2">Show Count</button>').insertAfter('#log-traders-table-btn');
    
    $(document).on('click', '#show-count-btn', function() {
        showTradersCount();
    });
    
    // Function to inspect table data structure
    function inspectTableData() {
        var table = $('#root-table').DataTable();
        if (table) {
            var data = table.data().toArray();
            var columns = table.settings().init().columns;
            
            console.log('=== TABLE DATA STRUCTURE INSPECTION ===');
            ...
            if (data.length > 0) {
                ...
                if ('ip' in firstRow) {
                    console.log('✅ IP column found in data');
                    console.log('IP value:', firstRow.ip);
                } else {
                    console.log('❌ IP column NOT found in data');
            ...
            // Check table headers
            var headers = [];
            $('#root-table thead th').each(function() {
    ...
    // Add button for table inspection
    $('<button id="inspect-table-btn" class="btn btn-secondary btn-sm ms-2">Inspect Table</button>').insertAfter('#show-count-btn');
    
    $(document).on('click', '#inspect-table-btn', function() {
        inspectTableData();
    });
    
    // Test function to manually check IP data
    function testIPData() {
        console.log('=== TESTING IP DATA ===');
        ...
        if (table) {
            ...
            if (data.length > 0) {
                ...
                console.log('Select column:', firstRow.select);
                console.log('Name column:', firstRow.name);
                console.log('IP column:', firstRow.ip);
                console.log('IP column type:', typeof firstRow.ip);
                console.log('IP column exists:', 'ip' in firstRow);
                
                // Check if IP is undefined, null, or empty
                if (firstRow.ip === undefined) {
                    console.log('❌ IP column is undefined');
                } else if (firstRow.ip === null) {
                    console.log('❌ IP column is null');
                } else if (firstRow.ip === '') {
                    console.log('❌ IP column is empty string');
                } else {
                    console.log('✅ IP column has value:', firstRow.ip);
                }
            }
        } else {
    ...
    // Add test button
    $('<button id="test-ip-btn" class="btn btn-danger btn-sm ms-2">Test IP Data</button>').insertAfter('#inspect-table-btn');
    
    $(document).on('click', '#test-ip-btn', function() {
        testIPData();
    });
    
    // Function to check AJAX response directly
    function checkAJAXResponse() {
        console.log('=== CHECKING AJAX RESPONSE ===');
        
        // Make a direct AJAX call to see the raw response
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    ...
                    console.log('First row from AJAX:', firstRow);
                    console.log('First row keys:', Object.keys(firstRow));
                    
                    // Check IP column specifically
                    if ('ip' in firstRow) {
                        console.log('✅ IP column found in AJAX response');
                        console.log('IP value:', firstRow.ip);
                        console.log('IP type:', typeof firstRow.ip);
                        console.log('IP length:', firstRow.ip ? firstRow.ip.length : 0);
                    } else {
                        console.log('❌ IP column NOT found in AJAX response');
                        console.log('Available columns:', Object.keys(firstRow));
                    }
    ...
    // Add AJAX check button
    $('<button id="check-ajax-btn" class="btn btn-dark btn-sm ms-2">Check AJAX</button>').insertAfter('#test-ip-btn');
    
    $(document).on('click', '#check-ajax-btn', function() {
        checkAJAXResponse();
    });
});
