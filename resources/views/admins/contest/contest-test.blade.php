...
<script>
    var dt = $('#ib_transfer_tbl').fetch_data({
        "url": "/admin/contest/contest-list?op=data_table",
        "columns": [{
                "data": "contest_name"
            },
            {
    ...
    $(document).on('click', '.btn-announce-result', function() {
        ...
        // First check if contest is closed
        $.ajax({
            url: '/admin/contest/check-contest-status',
            method: 'POST',
            data: {
                contest_id: contest_id,
            ...
            success: function(response) {
                if (response.status) {
                    if (response.contest_status === 'closed' || response.contest_status === 'ended' || response.contest_status === 'completed') {
                        // Contest is closed, proceed with result announcement
                        announceContestResult(contest_id);
                    } else {
                        // Contest is not closed
                        Swal.fire({
                            title: 'Contest Not Closed',
                            text: 'Please close the contest first before announcing results.',
                            icon: 'warning',
                            confirmButtonText: 'OK'
                        });
                    }
                } else {
                    notify('error', 'Error checking contest status', 'Announce Result');
                }
            },
    ...
    //Edit Contest
    $(document).on('click', '.btn-edit-contest', function() {
        let contest_id = $(this).data('contest_id');
    ...
    // Function to announce contest result
    function announceContestResult(contest_id) {
        // Get contest details and prizes
        $.ajax({
            url: '/admin/contest/get-contest-result-data',
            method: 'POST',
            data: {
                contest_id: contest_id,
                _token: $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(response) {
                if (response.status) {
                    showResultAnnouncementModal(response.data);
                } else {
                    notify('error', response.message || 'Error getting contest data', 'Announce Result');
                }
            },
            error: function(xhr, status, error) {
                notify('error', 'Error getting contest data', 'Announce Result');
            }
        });
    }
    
    // Function to show result announcement modal
    function showResultAnnouncementModal(data) {
        let modalHtml = `
            <div class="modal fade" id="resultAnnouncementModal" tabindex="-1" aria-labelledby="resultAnnouncementModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="resultAnnouncementModalLabel">Announce Contest Results</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-12">
                                    <h6 class="mb-3">Contest: ${data.contest_name}</h6>
                                    <div class="table-responsive">
                                        <table class="table table-bordered">
                                            ...
                                            <tbody id="resultTableBody">
                                                ${generateResultTableRows(data.winners)}
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-success" onclick="confirmAnnounceResult(${data.contest_id})">Announce Results</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $('#resultAnnouncementModal').remove();
        
        // Add modal to body
        $('body').append(modalHtml);
        
        // Show modal
        $('#resultAnnouncementModal').modal('show');
    }
    
         // Function to generate result table rows
     function generateResultTableRows(winners) {
         let rows = '';
    ...
    // Function to confirm and announce results
    function confirmAnnounceResult(contest_id) {
        $.ajax({
            url: '/admin/contest/announce-result',
            method: 'POST',
            data: {
                contest_id: contest_id,
                _token: $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(response) {
                if (response.status) {
                    $('#resultAnnouncementModal').modal('hide');
                    notify('success', response.message || 'Results announced successfully!', 'Announce Result');
                    
                    // Show results on contest leaderboard
                    showResultsOnLeaderboard(contest_id, response.data);
                } else {
                    notify('error', response.message || 'Error announcing results', 'Announce Result');
                }
            },
            error: function(xhr, status, error) {
                notify('error', 'Error announcing results', 'Announce Result');
            }
        });
    }
    
    // Function to show results on contest leaderboard
    function showResultsOnLeaderboard(contest_id, winners) {
        // This will trigger a notification on the contest leaderboard page
        // The leaderboard page will check for announced results and show them
        console.log('Results announced for contest:', contest_id, winners);
    }
</script>
