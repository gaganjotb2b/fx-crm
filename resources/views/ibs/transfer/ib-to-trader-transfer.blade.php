...
@section('title', 'IB to Trader Balance Transfer')
@section('page-css')
...
<div class="container-fluid py-4">
    <div class="row">
        <div class="col-12 text-center">
            <h3 class="mt-5">IB to Trader Transfer</h3>
                ...
                <div class="row">
                    <div class="col-12 col-lg-8 mx-auto my-5">
                        <div class="multisteps-form__progress">
                            ...
                            @endif
                            <!-- progress status -->
                            <button disabled class="multisteps-form__progress-btn" id="after-status" type="button" title="Order Info">
                                <span>Status</span>
                            </button>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12 col-lg-8 m-auto">
                        <div class="multisteps-form__form">
                            <form class="form-demo" action="{{ route('ib.transfer.ib-to-trader') }}" method="post" id="trader-transfer-form">
                                <!--single form panel-->
                                            ...
                                            <div class="col-12 col-sm-6 mx-auto mt-4 mt-sm-0 text-start">
                                                ...
                                                <div class="col-8 mx-auto">
                                                    <div class="alert alert-warning" role="alert">
                                                        <strong>Warning!</strong> You are not permited to IB to Trader! Please contact with <strong> {{config('app.name')}} </strong> Support.
                                                    </div>
                                                </div>
                                                @endif
                                            </div>
                                ...
                                <div class="card multisteps-form__panel p-3 border-radius-xl bg-white" data-animation="FadeIn">
                                    ...
                                    <div class="row">
                                        <div class="button-row d-flex mt-4 col-12">
                                            ...
                                            <button class="js-btn-next visually-hidden" type="button" id="btn-js-next-2">Next</button>
...
<script>
    ...
    function trader_transfer_call_back(data) {
        if (data.status === true && data.last_transaction) {
            // Update data
            $("#last-amount").text(data.last_transaction.amount);
            $("#last-txn-id").text(data.last_transaction.invoice_code || data.last_transaction.txnid);
    
            let statusText = "Pending", badgeClass = "dark";
            if (data.last_transaction.status === 'A') {
                statusText = "Approved"; badgeClass = "success";
            } else if (data.last_transaction.status === 'D') {
                statusText = "Declined"; badgeClass = "warning";
            }
    
            $("#last-status").html(`<span class="badge rounded-pill badge-${badgeClass} badge-sm">${statusText}</span>`);
    
            // 🔧 Force show final panel
            $(".multisteps-form__panel").removeClass("js-active");
            $(".multisteps-form__panel").last().addClass("js-active");
        }
    }
    // function trader_transfer_call_back(data) {
    ...
    //         return; // Stop further execution until OTP step is done
    //     }
    
    //     // Final step after full success
    //     if (data.status === true && data.last_transaction) {
    //         // Update values in final transaction status step
    //         $("#last-amount").text(data.last_transaction.amount);
    //         $("#last-txn-id").text(data.last_transaction.invoice_code || data.last_transaction.txnid);
    ...
    //         $("#last-status").html(
    //             `<span class="badge rounded-pill badge-${badgeClass} badge-sm">${statusText}</span>`
    //         );
    
    //         // Move to the final step
    //         $("#btn-js-next-2").trigger("click");
    
    //         notify('success', data.message, 'IB to Trader Fund Transfer');
    //     }
    
    //     // Error Handling
    //     if (data.status === false || data.valid_status === false) {
    //         notify('error', data.message, 'IB to Trader Fund Transfer');
