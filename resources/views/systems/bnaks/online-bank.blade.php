...
<div class="modal fade text-start modal-success" id="success" tabindex="-1" aria-labelledby="myModalLabel110" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{route('systems.online-bank.add')}}" method="post" id="form-add-new-bank" class="modal-content">
            @csrf
            <div class="modal-header">
                <h5 class="modal-title" id="myModalLabel110">Add new online bank</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="form-group mt-1">
                    <label for="bank-country">Bank Country</label>
                    <select name="bank_country" id="bank-country" class="select2 form-select">
                        <!-- country from select2 -->
                    </select>
                </div>
                <div class="form-group mt-1">
                    <label for="bank-name">Bank Name</label>
                    <input type="text" id="bank-name" class="form-input form-control" name="bank_name" placeholder="Bank name">
                </div>
                <div class="form-group mt-1">
                    <label for="bank-code">Bank Code</label>
                    <input type="text" id="bank-code" class="form-input form-control" name="bank_code" placeholder="Bank code">
                </div>
                <!-- currency -->
                <div class="form-group mt-1">
                    <label for="currency">Currency</label>
                    <input type="text" name="currency" id="currency" class="form-input form-control" placeholder="Currency">
                </div>
            </div>
            <div class="modal-footer mb-2">
                <button type="button" class="btn btn-success" id="btn-save-bank" data-el="fg" data-btnid="btn-save-bank" data-form="form-add-new-bank" data-loading="<div class='spinner-border spinner-border-sm' role='status'><span class='visually-hidden'>Loading...</span></div>" onclick="_run(this)" data-callback="add_bank_callback">
...
<script>
    ...
    if (dt_ajax_table.length) {
        ...
        var datatable = dt_ajax_table.DataTable({
            ...
            "ajax": {
                "url": "/system/banks/online-bank-list/get",
                "data": function(d) {
                    return $.extend({}, d, {
                        "month": $("#month").val(),
    ...
    // add new bank callback
    function add_bank_callback(data) {
        if (data.status) {
            notify('success', data.message, 'Add new Bank');
            datatable.draw();
        } else {
            notify('error', data.message, 'Add new Bank');
        }
