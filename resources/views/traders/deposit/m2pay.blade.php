...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
        ...
        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">{{__('page.crypto-deposit')}}</li>
    </ol>
...
<div class="container-fluid py-4">
    <div class="row custom-height-con">
        <div class="col-12 text-center">
            <h3 class="mt-5">Match2Pay Crypto Deposit</h3>
                                ...
                                <div class="multisteps-form__content">
                                    <div class="row mt-3">
                                        ...
                                        <div class="col-12 col-sm-6 mt-4 mt-sm-0 text-start pb-5">
                                            ...
                                            <form action="{{route('user.deposit.match2pay.send')}}" method="post" id="set-form-value">
                                                @csrf
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="crypto_symbol" id="crypto-symbol" class="form-select form-control">
                                                        <option value="">Choose a crypto symbol</option>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="currency" id="crypto-currency" class="form-control form-select">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" id="input-amount" name="usd_amount" data-symbol="USD" placeholder="0" class="form-control" style="color: black;">
                                                </div>
                                                <!-- crypto amount -->
                                                <div class="form-group">
                                                    <label for="local-currency">Amount( <span id="local-currency-label"></span> )</label>
                                                    <input type="text" id="local-currency" class="form-control" name="amount" value="0" placeholder="0" style="color: black;">
                                                </div>
                                                <div class="form-group">
                                                    <button type="button" data-label="Submit Request" id="btn-submit-request" data-btnid="btn-submit-request" data-callback="match2PayCallback" data-loading="<i class='fa-spinner fas fa-circle-notch'></i>" data-form="set-form-value" data-el="fg" onclick="_run(this)" class="btn bg-gradient-primary ms-auto mb-0 float-end">{{__('page.submit-request')}}</button>
                                                </div>
                                            </form>
                                            <!-- final form for api request -->
...
<script>
    ...
    $(document).on('change', '#crypto-symbol', function() {
        let symbol = $(this).val();
        $.ajax({
            url: '/user/crypto/crypto-currency/get-single',
            method: 'GET',
            dataType: 'JSON',
            data: {
                symbol: symbol,
            ...
            success: function(data) {
                ...
                var select_element = $("#crypto-currency");
                ...
                $("#local-currency-label").text($(select_element).val());
                $("#local-currency").data('symbol', $(select_element).val());
            }
        })
    });

    // conver amount 
    $.fn.convert_currency = function(options) {
        var settings = $.extend({
            target_id: "#input-amount",
        }, options)
        let $this = this;
        this.on('input', function() {
            $.ajax({
                url: '/user/crypto/crypto-currency/convert',
                method: 'GET',
                data: {
                    fsymbol: $($this).data('symbol'),
                    tsymbol: $(settings.target_id).data('symbol'),
                    amount: $($this).val(),
                },
                dataType: 'JSON',
                success: function(data) {
                    $(settings.target_id).val(data);
                }
            });
        })
    }
    // convert crypto to usd amount
    $("#local-currency").convert_currency({
        target_id: "#input-amount",
    });
    // convert usd to crypto amount
    $("#input-amount").convert_currency({
        target_id: "#local-currency",
    });
    ...
    function match2PayCallback(data) {

        if (data.status) {
            notify('success', data.message, 'Match2Pay Deposit');
            window.open(data.checkoutUrl, "_blank");
            setTimeout(() => {
                $("#btn-submit-request").prop('disabled', false);
            }, 5000);
        } else {
            notify('error', data.message, 'Match2Pay Deposit');
            setTimeout(() => {
                $("#btn-submit-request").prop('disabled', false);
            }, 1000);
        }
