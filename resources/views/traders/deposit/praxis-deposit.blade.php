    ...
    <div class="row custom-height-con">
        <div class="col-12 text-center">
            ...
            <div class="container mb-5">
                <div class="row">
                    <div class="col-12 col-md-6 card-body card row">
                        <div class="col-12 col-md-6 col-sm-6 mt-4 mt-sm-0 pb-5 mx-auto text-start ">
                            @if(\App\Services\AllFunctionService::kyc_required((auth()->user()->id),'deposit') == false )
                            <form action="{{route('user.deposit.praxis.request')}}" method="post" id="set-form-value">
                                @csrf
                                ...
                                <!--LOCAL-->
                                <div class="form-group">
                                    <label for="user-email">Local</label>
                                    <select id="payment-local" class="form-control" name="locale">
                                        <!--<option value="ar-SA">Arabic-Soudi Arabia</option>-->
                                ...
                                <!--currency-->
                                <div class="form-group">
                                    <label for="user-email">Currency</label>
                                ...
                                <!--amount-->
                                <div class="form-group">
                                    <label for="user-email">Amount (<span id="local-currency-label"></span> )</label>
                                    <input type="text" id="local-currency" class="form-control" placeholder="0" name="amount_local">
                                </div>
                                <div class="form-group">
                                    <label for="user-email">Amount(USD)</label>
                                    <input type="text" id="input-amount" class="form-control" data-symbol="USD" placeholder="0" name="amount">
                                </div>
...
<script>
    ...
    $(document).on('change', '#currency', function() {
        ...
        $("#local-currency").data('symbol', $(this).val());
    ...
    function match2PayCallback(data) {
        ...
        if (data.status) {
            notify('success', data.message, 'Praxis Deposit');
            window.open(data.redirect_url, "_self");
            setTimeout(() => {
                $("#btn-submit-request").prop('disabled', false);
            }, 5000);
        } else {
            notify('error', data.message, 'Praxis Deposit');
            setTimeout(() => {
                $("#btn-submit-request").prop('disabled', false);
            }, 1000);
        }
        $.validator("set-form-value", data.error);
    }
</script>
@stop