...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
        ...
        <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">{{__('Contest')}}</a></li>
...
<div class="container-fluid page-profile-overview">
    <div class="page-header min-height-300 border-radius-xl mt-4 bg-gradient-info">
        <div class="w-100">
            <h3 class="text-white h2 text-uppercase text-center w-100 z-index-1">{{$contest?->contest_name??'---'}}</h3>
            <div class="countdown-box pt-0" id="countdown">
                <p class="text-border text-light">Time remaining to close</p>
                <div data-ending="{{$contest?->contest_end_on??now()}}" id="countdown-time" class="countdown-time text-center z-index-3 d-flex justify-content-center gap-1">
                    <div class="day-container card px-4">
                        <span id="contest-days">0</span>
                        <span class="h4">Days</span>
                    </div>
                    <div class="hour-container card px-4">
                        <span id="contest-hours">0</span>
                        <span class="h4">Hrs</span>
                    </div>
                    <div class="minute-container card px-4">
                        <span id="contest-minutes">0</span>
                        <span class="h4">Min</span>
                    </div>
                    <div class="second-container card px-4">
                        <span id="contest-seconds">0</span>
            ...
            <div class="col-2 border-end border-primary">
                <div class="card bg-transparent shadow-none rounded-0">
                    <div class="card-body text-center">
                        <h4 class="text-bolder">Duration</h4>
                        <h4>{{$duration}}</h4>
                    </div>
                </div>
            </div>
            <div class="col-8 border-end border-primary">
                <div class="card bg-transparent shadow-none rounded-0">
                    <div class="card-body gap-2 text-center">
                        <h4 class="text-bolder">Prizes</h4>
                        @if (!empty($contest?->contestPrize))
                        @foreach ($contest->contestPrize as $prize)
                        <span class="badge-dark badge bg-gradient-info mt-2">
                            {{ $prize->prize }}.....
                            <span class="h4 text-light">$ {{ $prize->prize_value }}</span>
                        </span>
                        @endforeach
                        @endif


                        <!-- <span class="badge-secondary badge mt-2">2nd prize..... <span class="h4 text-dark">$5000</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$5000</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$5000</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$5000</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$5000</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$90</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$50</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$50</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$50</span></span>
                        <span class="badge-secondary badge mt-2">3rd prize..... <span class="h4 text-dark">$50</span></span> -->
                    </div>
...
<div class="container-fluid py-4">
        ...
        <div class="col-12 col-md-6 col-xl-4">
            ...
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <div class="row  position-relative mb-3" id="trading-ac-data-list">
                        <h4 class="h4" style="font-weight: 700;">How to join this contest, follow this bellow instructions</h4>
            ...
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <h6 class="h6" style="font-weight: 700;">
                        Start on: <span class="text-bold">{{ \Carbon\Carbon::parse($contest?->contest_start_on)->format('d M Y') }}</span>
                    </h6>
                    <h6 class="h6" style="font-weight: 700;">
                        Ends on: <span class="text-bold">{{ \Carbon\Carbon::parse($contest?->contest_end_on)->format('d M Y') }}</span>
                    </h6>
                            ...
                            <!-- <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="ms-3">
                                            <h4 class="mb-0 h5">Reza</h4>
                                            <p class="text-sm mb-0 text-capitalize font-weight-bold">India</p>
                                        </div>
                                    </div>
                                </td>
                                <td>120</td>
    ...
    <!-- modal for join contest -->
    <div class="modal fade" id="modal-join-contest" tabindex="-1" role="dialog" aria-labelledby="join-contest" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="popup-header">Join Contest</h5>
                    <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('users.contest.join') }}" method="post" id="form-join-contest">
                        @csrf
                        <input type="hidden" id="hidden_input_contest_id" name="contest" value="{{$contest?->id}}">
                        <p>Please choose an account join the contest, this is account base contest so for join you need a trading account.</p>
                        <div class="form-group">
                            <label for="recipient-name" id="social_link" class="col-form-label">{{ __('Account') }}:</label>
                            <select name="account" id="contest-account" class="form select form-control">
                                <option value="">Choose an account</option>
                                @foreach ($accounts as $account)
                                <option value="{{$account->id}}">#{{$account?->account_number}}({{$account?->client_type}})</option>
                                @endforeach
                            </select>
                        </div>
                        <table>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" data-label="Update" id="btn-submit-request" data-btnid="btn-submit-request" data-callback="contestJoinCallback" data-loading="<i class='fa-spinner fas fa-circle-notch'></i>" data-form="form-join-contest" data-el="fg" onclick="_run(this)" class="btn bg-gradient-primary">{{ __('page.submit') }}</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        function updateCountdown() {
            ...
            $('#contest-days').text(days);
            $('#contest-hours').text(hours);
            $('#contest-minutes').text(minutes);
            $('#contest-seconds').text(seconds);
        }
        ...
        $(document).on('click', "#btn-join-modal-show", function() {
            $("#modal-join-contest").modal('show');
        });
        // change account / show balance
        $(document).on('change', "#contest-account", function() {
            let account_id = $(this).val();
            $.ajax({
                url: "/user/contest/leaderboard/balance-equity",
                method: 'GET',
                data: {
        ...
        // datatable
        const deposit_report = $("#dt-leaders").DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            lengthChange: false,
            ...
            pageLength: 10,
            ajax: {
                url: "/user/contest/leaderboard/leaders",
                data: function(d) {
                    return $.extend({}, d, {
                        'contest': "{{$contest?->id}}"
                    });
    ...
    function contestJoinCallback(response) {
        if (response.status === true) {
            notify('success', response.message, 'Join Contest');
        } else {
            notify('error', response.message, 'Join Contest');
        }
        $.validator("form-join-contest", response.errors);
