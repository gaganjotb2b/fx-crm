...
@section('title','Trader to IB Transfer')
@section('page-css')
...
<div class="container-fluid py-4">
    <div class="row custom-height-con">
        <div class="col-12 text-center">
            <h3 class="mt-5">{{ __('page.trader-to-ib-transfer') }}</h3>
            <h5 class="text-secondary font-weight-normal">{{ __('page.we-need-some-info') }}</h5>
                ...
                <div class="row">
                    <div class="col-12 col-lg-8 m-auto">
                        <div class="multisteps-form__form bg-custom-dark-for rounded-3">
                            <form class="form-demo" action="{{route('user.transfer.trader-to-ib-transfer-form')}}" method="post" id="trader-transfer-form">
                                <!--single form panel-->
                                <div class="card multisteps-form__panel p-3 border-radius-xl bg-white js-active" data-animation="FadeIn">
                                    <div class="row text-center">
                                        <div class="col-10 mx-auto">
                                            <h5 class="font-weight-normal">{{ __('page.let\'s-start-with-your-account-information') }}</h5>
                                            <p>{{ __('page.the-ib-email-amount-and-your-transaction-password-required-for-your-secure-transaction') }}</p>
                                        </div>
                                    </div>
                                    <div class="multisteps-form__content">
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-12 col-sm-6 mx-auto mt-4 mt-sm-0 text-start">
                                                <!-- check trader to ib transfer permited -->
                                                @if(\App\Services\Trader\FinanceService::check_op('trader_to_ib'))
                                                <input type="hidden" name="op" id="op" value="step-1">
                                                @csrf
                                                <div class="form-group">
                                                    <label for="recipient">{{ __('page.ib_email') }}</label>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" class="form-control multisteps-form__input" id="amount" name="amount">
                                                </div>
                                                <div class="form-group">
                                                ...
                                                <div class="col-8 mx-auto">
                                                    <div class="alert alert-warning" role="alert">
                                                        <strong>Warning!</strong> You are not permited to Trader to IB! Please contact with <strong> {{config('app.name')}} </strong> Support.
                                                    </div>
                                                </div>
                                                @endif
                                            </div>
                                        ...
                                        <div class="button-row d-flex mt-4">
                                            ...
                                            <div class="col-6 mx-auto">
                                                ...
                                                @if(\App\Services\Trader\FinanceService::check_op('trader_to_ib'))
                                                <button type="button" data-label="Next" id="btn-submit-request" data-btnid="btn-submit-request" data-callback="trader_transfer_call_back" data-loading="<i class='fa-spin fas fa-circle-notch'></i>" data-form="trader-transfer-form" data-el="fg" onclick="_run(this)" class="btn bg-gradient-primary ms-auto mb-3 mt-4 float-end" data-submit_wait="{{submit_wait('atw-transfer',60)}}" {{$disable}} style="width: 200px">{{ __('page.next') }}</button>
                                                @endif
                                            </div>
...
<script>
    ...
    function trader_transfer_call_back(data) {
        if (data.status == true) {
            notify('success', data.message, 'Trader to IB Fund Transfer');
            $("#trader-transfer-form").trigger('reset');
        ...
        if (data.status == false) {
            notify('error', data.message, 'Trader to IB Fund Transfer');
            $.validator("trader-transfer-form", data.errors);
        }
        // validation status
